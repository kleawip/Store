...
### 2026-09-25 (late) — Codex (catalogue-backed product pages and videos)
- **Changed:** `apps/storefront` product detail route now fetches and validates `GET /v1/store/products/{slug}` per request. API title, category, description, photos and approved price take precedence over local fixtures. Published `ProductDetail.videos[]` render on the relevant product page, with hosted playback, muted in-view autoplay, sound and seek controls; verified Instagram embeds have a fallback link. The Twisted Loop local reel appears only as a clearly labelled demo when that demo product has no published video. API 404 does not revive an unpublished fixture; server failures can still show a labelled local preview. No backend files were edited.
- **Verification:** storefront typecheck, 30/30 tests and production build passed. Browser QA on the live API-backed Twisted Loop page at 320, 390, 768 and 1280 px found no horizontal overflow; the 390 px page and reel section were visually inspected. Published-video override and no-video conditions were unit-tested. Authenticated admin publish → storefront playback still requires owner sign-in and an approved media asset; soundtrack rights remain unconfirmed. The local bag/checkout remains a separate phase.
- **Coordination:** Claude agreed to this product-page scope by mailbox and confirmed codec validation plus the owner-only embed gate are fixed. `graphify update .` ran. Graphify output includes concurrent Claude Milestone 3 edits and was not staged in the frontend commit.
...
### 2026-09-25 — Claude Code (Milestone 4: discount codes and reports)
- **Changed:** `services/`, `database/` (migration 0017), `packages/contract` (additive), API_CONTRACT §5.4. Discount codes: percentage (capped), fixed amount, free shipping. Limits: minimum spend, dates, total uses, per-customer uses, first order only. Applied at checkout, spread across lines so GST is charged on the discounted value, and re-checked with the code locked at order placement (the last use can't be taken twice). Invoices, credit notes, return refunds and courier bookings use the discounted values. Owner-only reports: sales by day/month (net of refunds), top products, and a GST summary (HSN, place of supply, document register) with CSV export. New permissions: `discounts.read` (owner, marketing, operations, support), `discounts.manage` and `reports.read` (owner).
